...
const CaseStudies = async ({ slice }: CaseStudiesProps): Promise<JSX.Element> => {
  ...
  return (
    ...
    >
<h2 className="max-w-2xl text-balance text-center text-5xl font-medium
md:text-7xl">
...
{caseStudies.map((caseStudy, index) => (
  caseStudy && (
  <div key={caseStudy.id} className="relative grid gap-4 opacity-85 transition-opacity 
  duration-300 hover:cursor-pointer hover:opacity-100 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
    ...
    <PrismicNextLink document={caseStudy} className="after:absolute after:inset-0 
    hover:underline" >
